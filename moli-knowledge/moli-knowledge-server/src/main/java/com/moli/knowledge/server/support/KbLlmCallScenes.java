package com.moli.knowledge.server.support;

/**
 * kb_llm_call_log.scene 常量。
 */
public final class KbLlmCallScenes {

    public static final String ASK = "ask";
    public static final String INGEST_PLAN = "ingest_plan";
    public static final String INGEST_GENERATE = "ingest_generate";
    public static final String INGEST_ENRICH = "ingest_enrich";
    public static final String WIKI_REVISE = "wiki_revise";
    public static final String WIKI_ENRICH = "wiki_enrich";
    public static final String LLM_TEST = "llm_test";

    private KbLlmCallScenes() {
    }
}
