...
5. 若答案综合多页形成新洞见 → 建议 crystallize 到 `wiki-moli/develop/outputs/`（需人工确认）
...
### 3.3 Web 体检检查项（DB 侧 · KBOPS-8/10）

| 结果字段 | `issue_type` | 含义 | 与 `lint.py` |
|----------|--------------|------|--------------|
| `broken` | `broken_link` | 正文 `[[目标]]` 在库内解析不到 | 类似；CLI 还扫 related/edges，且忽略代码块内 `[[..]]` |
| `orphans` | `orphan` | 无入链（含同标签） | CLI 还把 `index.md` 引用算作入链 |
| `noSummary` | `no_summary` | DB `summary` 为空 | **Web 特有**；CLI 查 frontmatter `sources` |
| `duplicates` | `duplicate` | slug 裸名歧义 | `dup_slug` |
| `stale` | `stale` | 库龄 / `supersedes` 仍发布 | `outdated` + 库龄 |
| `conflicts` | `conflict` | `contentHash` 相同 | `dup_content`（CLI 需 `--dups`） |
| `missingSources` | `missing_source` | frontmatter `sources` 为空 | 一致 |
| `badTypes` | `bad_type` | frontmatter `type` 不在白名单 | 一致 |
| `missingTitles` | `missing_title` | 无 title 且无 H1 | 一致 |
| `slugMismatches` | `slug_mismatch` | frontmatter slug ≠ 路径末段 | 一致 |
| `missingDates` | `missing_dates` | 缺 `created`/`updated` | 一致 |
| `missingConcepts` | `missing_concept` | 断链目标被 ≥3 页引用 | 一致（默认阈值 3） |

**仍以 `lint.py --strict` 为准（文件真值 / CI 门禁）**：`near_dup`、`asym_related`、`space_branding`、代码块内示例链接过滤、全量 `related`/edges 断链等。  
**Wiki 治理页**（`lint-space` + 脚本修复）负责磁盘 `.md` 批量修 frontmatter。
...
6. **（可选）漂移检测**：`GET /kb/sync/drift?spaceId=` 或 `python kb/tools/detect_wiki_db_drift.py`（KBOPS-A3）
...
5. **Query** 验证答案；复杂结果可写 `wiki-moli/develop/outputs/` 并在 `log.md` 记 `query | crystallize`

> **AI 审校单篇 MD 再 Sync** 的逐步说明（Cursor Agent 模板 + `lint.py --strict` 门禁）见 wiki [[AI自我进化与MD审校流程]]（`moli-knowledge/kb/wiki-moli/develop/AI自我进化与MD审校流程.md`）。
...
| 项 | 结果 | 说明 |
|----|------|------|
| **断链** | 11（误报 11） | 文档中 meta 示例 `[[slug]]`、`[[知识库设计哲学-docs-as-code]]`、`[[..]]` 非真实链接，可忽略 |
| **孤儿页** | 2 → **已修** | `swagger接口调试指南`、`shiro-面试题` 已从 [[本地启动指南]]、[[认证与会话机制]] 入链 |
