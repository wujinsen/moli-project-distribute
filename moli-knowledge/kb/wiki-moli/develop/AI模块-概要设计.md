...
tags: [设计, 概要设计, 架构]
sources:
  - docs/design/ai-module-overview.md
related:
  - bi服务
  - 技术方案与架构索引
created: 2026-07-06
...
> **浏览镜像**：工程契约权威仍在 `docs/design/ai-module-overview.md`；改设计请先改契约再重新运行本脚本或 Tab3 导入。

> 模块：**`moli-ai`**（Maven 名）· Nacos 服务名 **`ai-server`** · HTTP **1128**  
> v1 范围：`docs/product/moli-v1-release-scope.md` §3.5  
> API：`docs/api/ai-api.md` · 冒烟：`docs/test/ai-smoke.md`
...
鉴权策略与 order 一致：**Session 在 user-center 签发**，AI 只做校验。
...
| API | `docs/api/ai-api.md` |
| 测试 | `docs/test/ai-smoke.md` |
| wiki 服务 | [[bi服务]] |
