...
> **浏览镜像**：工程契约权威仍在 `docs/design/kb-ops-roadmap.md`；改设计请先改契约再重新运行本脚本或 Tab3 导入。

> 更新：2026-07-02 · 状态：**规划**（部分后端已就绪）
> 归属：`moli-knowledge` · `kb_sync_log` / `kb_lint_issue` / `kb_platform_llm_config`
> 边界：**只管知识库内容管道运维**（wiki→DB 同步、体检、LLM 配置）；服务器/基础设施运维见 [[服务器运维模块规划]]（另一条独立路线，互不重叠）
...
| K-P4 | **定时同步默认关闭 + 仅单空间**：`schedule-enabled=false`，且只 sync `space-code`（enterprise-kb），不是 sync-all 两空间 | 🟡 中 | `application-dev.yml` |
...
| 任务 | 内容 |
|------|------|
| **KBOPS-4** | 定时同步改 **sync-all 两空间**（或配置化 `space-codes` 列表）；文档与 `KbSyncScheduler` 对齐 |
...
| **KBOPS-9** | **知识库运维 Dashboard**：Sync 批次趋势、Lint 工单、LLM 调用/失败率、漂移采样 | ✅ 后端 |
| **KBOPS-10** | Web 健康体检检查项对齐 `lint.py` + 分工文档 | ✅ |
...
| `kb_lint_issue` | 体检工单 | ✅ KBOPS-8 assignee/priority + 批量 API |
...
- **不含服务器/基础设施运维**（平台账号、服务器、组件台账）—— 见 [[服务器运维模块规划]]。
