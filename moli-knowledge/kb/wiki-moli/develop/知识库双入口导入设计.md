...
| 导入包内插图自动回迁 | MVP 只写 md 文本；含 `
> **架构图**：`` 相对路径时 lint 告警，T22 `.assets` 批量复制放 P1 |
...
moli-knowledge-server :8090
  ...
  KbSyncService               ← 复用 trigger（T3）
       ...
       ▼
sync_to_db.py / POST /kb/sync/trigger → MySQL kb_document (source=kb`（请在仓库中打开 PNG；源文件见同目录 `.drawio`）
...
> **双入口导入流程**：`docs/diagrams/png/moli-kb-import-entry.png`（请在仓库中打开 PNG；源文件见同目录 `.drawio`）


> 源文件：`docs/diagrams/moli-kb-import-entry.drawio`

### 2.4 API 时序（实现参考）


> **T20 API 时序**：`docs/diagrams/png/moli-kb-import-entry-api.png`（请在仓库中打开 PNG；源文件见同目录 `.drawio`）


> 源文件：`docs/diagrams/moli-kb-import-entry-api.drawio`
...
|------|----------|---------|
| md 内 `
> **架构图**：`http...`（请在仓库中打开 PNG；源文件见同目录 `.drawio`）
` | 允许；Web 浏览依赖 T22 F1 | — |
| md 内 `
> **架构图**：`foo.png`（请在仓库中打开 PNG；源文件见同目录 `.drawio`）
` 相对路径 | lint-preview **WARN**；不自动复制 | zip 附带 `.assets/` 一并 import |
