...
tags: [设计, 概要设计, 架构]
sources:
  - docs/design/kb-llm-platform-settings.md
related:
  - 知识库模块-概要设计
  - 知识库服务
  - 技术方案与架构索引
...
> API 契约（规划）：`docs/api/KNOWLEDGE_API.md` §3.5  
> DDL：`docs/sql/11_kb_platform_llm_config.sql`  
> 任务跟踪：`moli-knowledge/TASKS.md` **T19**
...
> 实现后写入 `docs/api/KNOWLEDGE_API.md` §3.5 为权威。
...
> **联调权威文档**：`docs/api/kb-llm-platform-frontend.md`（TypeScript 类型、API 封装、验收清单、联调步骤）
...
`system.kbLlm.*`（zh/en/ja），与 `knowledge.ask.llm` 探测文案区分。键表见 `docs/api/kb-llm-platform-frontend.md` §12。
...
- `moli-knowledge/kb/wiki-moli/develop/Ingest工作台产品方案.md` §3.3 Express（依赖平台 LLM）
- [[用户中心-详细设计]]（系统管理菜单 / RBAC 模式）