...
> 目标：在 **Web 界面** 打开 wiki 页，**调用已配置的 LLM** 协助改稿；展示 **修改前/后对比**；支持 **人工继续改**、**Enrich 追加章节**；确认后 **保存回 `kb/wiki-moli/*.md`**，再 Sync 进库。
...
| AI 改 MD | §4 **场景 B** 审校规则 + §6 Cursor 模板 | 同一审校规则 → `POST /kb/wiki-moli/ai-revise` 的 system prompt |
...
- **生成**：调用后端 `POST /kb/wiki-moli/ai-revise` → 返回 **建议全文**（含 frontmatter）
- **应用方式**：
...
- **流程**：**预览合并**（`POST /kb/wiki-moli/enrich` + `dryRun: true`）→ 确认 diff → **应用 Enrich**（写盘）
...
2. 用户确认 → `PUT /kb/wiki-moli/page` 写文件
3. 可选：**保存并 Sync**（§步骤 4）
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/kb/wiki-moli/page?slug=&spaceId=` | 读 wiki 文件全文（frontmatter+body）；需空间 **editor** |
| PUT | `/kb/wiki-moli/page` | 写 wiki 文件；body：`slug, spaceId, content, changeLog?`；需 editor |
| POST | `/kb/wiki-moli/ai-revise` | AI 改稿建议；见下表 |
| POST | `/kb/wiki-moli/page/lint-preview` | 保存前轻量 lint 摘要 |
| POST | `/kb/wiki-moli/enrich` | **Enrich 治理**：追加 patch + 可选 log/index/edges；见 `docs/api/KNOWLEDGE_API.md` §8.4 |
| GET | `/kb/wiki-moli/page/history?slug=` | 🔜 P2：git 历史或版本表（可选） |

### 3.1 `POST /kb/wiki-moli/ai-revise`
...
```
Web 编辑保存
    → 写服务器 kb/wiki-moli/{type}/{slug}.md  （权威源）
    → （用户或勾选）Wiki 同步
    → kb_document 更新
```
...
| **T14f** | `POST /kb/wiki-moli/enrich` + `enrich.py` CLI + 编辑页 **Enrich 治理** 侧栏（预览/应用、log/index/edges） | 单页 enrich 与 Ingest/CLI 语义一致 |
...
[[AI自我进化与MD审校流程]] · [[项目文档总览]] · [[项目文档总览]] · [[知识库三操作]]
