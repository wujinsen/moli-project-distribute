...
def _repair_compound_counts(counter: Counter[str]) -> Counter[str]:
    ...
    repairs: dict[str, list[str]] = {
        ...
        "データマイニングビッグデータ": ["データマイニング", "ビッグデータ"],
        "データクレンジングデータベース": ["クレンジング", "データベース"],
        "データディクショナリシステム": ["データディクショナリ", "システム"],
        "カスタマリレーションシップマネジメント": ["カスタマーリレーションシップマネジメント"],
        "ロールバックトランザクション": ["ロールバック", "トランザクション"],
        "チェックポイントデータベース": ["チェックポイント", "データベース"],
        "バックアップファイルデータベース": ["バックアップファイル", "データベース"],
        "ワークファイルジョブ": ["ワークファイル", "ジョブ"],
        "ロールフォワードデータベース": ["ロールフォワード", "データベース"],
        "ロールアウトメモリ": ["ロールアウト", "メモリ"],
    }
    drop_only = {
        "セキュリティサーティファイ",
