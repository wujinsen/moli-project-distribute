# -*- coding: utf-8 -*-
"""Generate マネジメント markdown + 中文解析 from Moodle HTML (10 Q, mapped from マネジメント・ストラテジ)."""
from __future__ import annotations

import re
import sys
from datetime import date
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent))
from moodle_quiz_html_to_md import html_to_markdown, _parse_questions, _parse_summary  # noqa: E402
from certify_mgmt_analyses import ANALYSES as COMBINED  # noqa: E402
from bs4 import BeautifulSoup  # noqa: E402

KB = Path(__file__).resolve().parents[1]
DIR = KB / "raw/school/certify"
STEM = "マネジメント"
TITLE = "マネジメント"
HTML = DIR / f"{TITLE}_ Attempt review.html"
MD_OUT = DIR / f"{STEM}.md"
ZH_OUT = DIR / f"{STEM}_中文解析.md"

# 本卷 10 题与 combined マネジメント・ストラテジ 卷内题号对应（题干相同、题序不同）
MGMT_TO_COMBINED: dict[int, int] = {
    1: 13,
    2: 17,
    3: 24,
    4: 1,
    5: 18,
    6: 32,
    7: 15,
    8: 8,
    9: 30,
    10: 4,
}


def _combined_num(qnum: int) -> int | None:
    return MGMT_TO_COMBINED.get(qnum)


def _parse_html_questions() -> list[dict]:
    soup = BeautifulSoup(HTML.read_text(encoding="utf-8", errors="replace"), "html.parser")
    qs = _parse_questions(soup)
    for que, q in zip(soup.select("div.que"), qs):
        gf = que.select_one(".generalfeedback")
        if gf:
            q["general_feedback"] = gf.get_text("\n", strip=True)
    return qs


def _stem_zh(qnum: int, q: dict) -> str:
    cn = _combined_num(qnum)
    if cn and cn in COMBINED:
        return COMBINED[cn]["stem_zh"]
    text = re.sub(r"!\[[^\]]*\]\([^)]+\)", "[图]", q.get("text") or "")
    return text[:500] + ("..." if len(text) > 500 else "")


def _analysis_body(qnum: int, q: dict) -> str:
    cn = _combined_num(qnum)
    if cn and cn in COMBINED:
        body = COMBINED[cn]["body"].strip()
        gf = q.get("general_feedback", "")
        if gf and "官方反馈" not in body:
            body += "\n\n**官方反馈（日文）：**\n" + gf
        return body
    gf = q.get("general_feedback", "")
    ans = q.get("correct_answer", "")
    lines = [f"**正确答案：** {ans}", ""]
    if gf:
        lines.extend(["**官方反馈（日文）：**", gf, ""])
    lines.extend(["**考点：** 见日文题干。", "", f"**详细推导：** 请对照 `{STEM}.md` 验算。"])
    return "\n".join(lines)


def _fix_md_frontmatter(md: str) -> str:
    today = date.today().isoformat()
    return re.sub(
        r"(?s)^---\n.*?^---\n",
        (
            "---\n"
            f"title: {TITLE}\n"
            f"slug: {STEM}\n"
            "type: exam\n"
            "status: active\n"
            "tags: [certify, サーティファイ, moodle, マネジメント]\n"
            "sources:\n"
            f"  - kb/raw/school/certify/{TITLE}_ Attempt review.html\n"
            f"created: {today}\n"
            f"updated: {today}\n"
            "---\n"
        ),
        md,
        count=1,
    )


def build_zh_md(questions: list[dict], summary: dict) -> str:
    today = date.today().isoformat()
    grade = summary.get("Grade", "0%").replace("**", "")
    n = len(questions)
    lines = [
        "---",
        f"title: サーティファイ対策 {TITLE} 中文解析",
        f"slug: {STEM}-中文解析",
        "type: exam-analysis",
        "status: active",
        "tags: [certify, サーティファイ, 中文解析, マネジメント]",
        "sources:",
        f"  - kb/raw/school/certify/{STEM}.md",
        f"  - kb/raw/school/certify/{TITLE}_ Attempt review.html",
        "related: [マネジメント_ストラテジ-中文解析, ストラテジー-中文解析, サーティファイ対策]",
        f"created: {today}",
        f"updated: {today}",
        "---",
        "",
        f"# サーティファイ {TITLE} · 中文详细解析",
        "",
        f"> 对应原题：`{STEM}.md` 及 Moodle 回顾 HTML。",
        f"> 本卷 **{n} 题**，与 `マネジメント_ストラテジ` 卷部分题目相同（题序不同）。",
        "",
        "## 测验摘要",
        "",
        "| 项目 | 内容 |",
        "|------|------|",
        f"| 题数 | {n} |",
        f"| 本次得分 | {grade} |",
        "| 用途 | Certify マネジメント 专题 |",
        "",
    ]
    for q in questions:
        qnum = int(q["number"])
        lines.extend(["---", "", f"## 第 {qnum} 题", "", "### 日文题干", ""])
        if q.get("text"):
            lines.append(q["text"])
        else:
            lines.append("（见 HTML）")
        lines.extend(["", "### 中文题意", "", _stem_zh(qnum, q), ""])
        if q.get("options"):
            lines.extend(["### 选项", ""])
            for opt in q["options"]:
                marks_m = []
                if opt.get("selected"):
                    marks_m.append("已选")
                if opt.get("is_correct"):
                    marks_m.append("正解")
                suffix = f"（{', '.join(marks_m)}）" if marks_m else ""
                lbl = opt.get("label") or "?"
                lines.append(f"- **{lbl}.** {opt.get('text', '')}{suffix}")
            lines.append("")
        cn = _combined_num(qnum)
        ans = (COMBINED.get(cn, {}).get("answer") if cn else None) or q.get("correct_answer", "")
        lines.extend(["### 正确答案", "", f"**{ans}**", "", "### 解析", "", _analysis_body(qnum, q), ""])
    lines.extend([
        "---",
        "",
        "## 复习建议",
        "",
        "1. 与 `マネジメント_ストラテジ` 卷对照：箭线图/关键路径、WBS/甘特、FP/TCO、SLA、备份磁带、DB 再編成。",
        "2. 进度偏差：计划速率 vs 实际完成量 → 落后天数。",
        f"3. 更新 HTML 后：`python kb/tools/gen_certify_management_analysis.py`。",
        "",
    ])
    return "\n".join(lines)


def main() -> None:
    soup = BeautifulSoup(HTML.read_text(encoding="utf-8", errors="replace"), "html.parser")
    summary = _parse_summary(soup)
    md = _fix_md_frontmatter(
        html_to_markdown(HTML, source_rel=f"kb/raw/school/certify/{TITLE}_ Attempt review.html")
    )
    md = md.replace("# サーティファイ対策", f"# {TITLE}")
    MD_OUT.write_text(md, encoding="utf-8")
    print(f"[ok] {MD_OUT.name}")
    questions = _parse_html_questions()
    zh = build_zh_md(questions, summary)
    ZH_OUT.write_text(zh, encoding="utf-8")
    mapped = sum(1 for q in questions if _combined_num(int(q["number"])) in COMBINED)
    print(f"[ok] {ZH_OUT.name} ({len(questions)} questions, mapped analyses={mapped})")


if __name__ == "__main__":
    main()
