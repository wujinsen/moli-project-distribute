# -*- coding: utf-8 -*-
"""Generate 模擬問題5 markdown + 中文解析 from Moodle HTML."""
...
def build_zh_md(questions: list[dict], summary: dict) -> str:
    ...
    lines = [
        ...
        "related: [模擬問題1-中文解析, 模擬問題2-中文解析, 模擬問題3-中文解析, サーティファイ対策]",
        f"created: {today}",
        f"updated: {today}",
        "---",
        "",
        ...
        "",
        f"> 对应原题：`模擬問題{EXAM}.md` 及 Moodle 回顾 HTML（含 `模擬問題{EXAM}_ Attempt review_files/` 插图）。",
    ...
    lines.extend([
        ...
        "## 复习建议",
        "",
        "1. 本卷覆盖 **数制/浮点/算法/CPU/缓存/OS/DB/网络/SE/经营**；建议分块重做并对照 `_files` 插图手算。",
        "2. Q22、Q39、Q47、Q49 等计算题务必代入题面数字验算。",
        f"3. 更新 HTML 后重新运行：`python kb/tools/gen_certify_q{EXAM}_analysis.py`。",
        "",
    ])
    return "\n".join(lines)
