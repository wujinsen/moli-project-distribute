...
related: [知识库使用指南, 知识库服务, 数据库初始化指南, 知识库三操作, 系统操作手册入口]
created: 2026-06-22
updated: 2026-06-25
---
...
|-------------------------|--------------|------------|------|------------|
| `wiki/` | `enterprise-kb` | `900000000000000001` | 技术文章、概念、面试题、知识库治理 | `docs/sql/03_knowledge_schema.sql`（内置种子） |
| `wiki-ops/` | `moli-ops-manual` | `900000000000000003` | 系统部署、启动、鉴权、联调、故障排查 | `docs/sql/07_kb_space_ops_manual.sql` |
| `wiki-jp-exam/` | `jp-fe-ap-exam` | `900000000000000002` | 日本語 FE/AP 题库 | `docs/sql/04_kb_space_jp_exam.sql` |

**规则**：

- 每篇 guide **只活在一个 wiki 目录 / 一个空间**（运维类只在 `wiki-ops/`，见 enterprise-kb 内 [[系统操作手册入口]] 跳转说明）。
...
| `wiki-ops/guides/本地启动指南.md` | `guides/本地启动指南` |
| `wiki-ops/develop/用户中心.md` | `develop/用户中心` |
| `wiki-ops/ops/wiki同步指南.md` | `ops/wiki同步指南` |
...
等价于对 `wiki` / `wiki-ops` / `wiki-jp-exam` 各执行一次 `--dry-run`。
...
# moli-ops-manual
python moli-knowledge/kb/tools/sync_to_db.py \
  --wiki-dir wiki-ops --space moli-ops-manual \
...
| 改了 ops 页但 Web 看不到 | 是否只 sync 了 `enterprise-kb`；应 `--wiki-dir wiki-ops --space moli-ops-manual` 或 `sync-all` |
...
- enterprise-kb 内跳转说明：`kb/wiki/guides/系统操作手册入口.md`（含三空间映射摘要）
