...
source_pages: [本地启动指南, 数据库初始化指南, 登录与鉴权指南, 前端开发与联调指南, 知识库使用指南, 故障排查指南, 茉莉微服务全链路一张图]
sources:
  - wiki/guides/本地启动指南.md
  - wiki/guides/数据库初始化指南.md
  - wiki/guides/登录与鉴权指南.md
  - wiki/guides/前端开发与联调指南.md
  - wiki/guides/知识库使用指南.md
  - wiki/guides/故障排查指南.md
  - wiki/outputs/茉莉微服务全链路一张图.md
related: [本地启动指南, 登录与鉴权指南, 茉莉微服务全链路一张图, 故障排查指南, 知识库使用指南, 前端开发与联调指南, 数据库初始化指南]
created: 2026-06-22
updated: 2026-06-22
---
...
| # | 动作 | 参考 |
|---|------|------|
| 1 | `POST http://localhost:21000/UserCenter/login/...`（演示账号见 [[数据库初始化指南]]） | [[登录与鉴权指南]] |
| 2 | 复制响应中的 token | |
| 3 | 请求业务接口时加 `Authorization: <token>` | |
| 4 | 或用 Swagger：`http://localhost:8888/swagger-ui.html` 等 | [[swagger接口调试指南]] |

网关 **不验 token**，鉴权在业务服务 Shiro 层 [[spring-cloud-gateway]]。
...
| 2 | `python moli-knowledge/kb/tools/sync_to_db.py --dry-run` | [[wiki同步指南]] |
| 3 | 写库后 `GET /kb/page?slug=guides/本地启动指南` | |
| 4 | 本地 Viewer：`python kb/tools/serve.py` → `:8765` | [[查询与体检指南]] |
...
| Dubbo No provider | Nacos、启动顺序 [[故障排查指南]] |
| 浏览器 CORS | 网关 CORS 或 devServer proxy [[跨域与前后端分离]] |

---

## 8. 源页索引

[[本地启动指南]] · [[数据库初始化指南]] · [[登录与鉴权指南]] · [[权限管理操作指南]] · [[故障排查指南]] · [[茉莉微服务全链路一张图]]
