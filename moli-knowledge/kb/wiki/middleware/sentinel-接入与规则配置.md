...
> 概念枢纽 [[middleware/sentinel-限流与熔断]]；路由 [[middleware/gateway-路由与过滤器]]；Dubbo [[middleware/dubbo-与-nacos]]。
...
与 [[middleware/gateway-路由与过滤器]] 中 `RequestRateLimiter`（Redis）二选一或叠加；Sentinel 规则可动态推送。
...
- [ ] 规则持久化到 Nacos，重启不丢
- [ ] 与 [[middleware/压测监控与prometheus]] 联调：限流后 Druid `waiting` 应下降
