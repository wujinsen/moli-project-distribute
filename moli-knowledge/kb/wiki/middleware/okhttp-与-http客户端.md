...
> Feign 底层 [[middleware/openfeign-与-http客户端]]；响应式 [[middleware/webclient-与-resttemplate]]；TLS [[security/https与-tls基础]]。
...
- **Interceptor** 统一 traceId [[java/mdc-日志链路上下文]]

## 3. 与 Dubbo

内部 RPC 优先 **Dubbo** [[middleware/dubbo-调用原理与分层]]；OkHttp 用于第三方 HTTP、Webhook、压测脚本。

## 4. 踩坑

- 未关闭 `Response.body()` → 连接泄漏（类似 JDBC [[database/druid-连接池泄漏排查]]）
- 同步调用占 Tomcat 线程 → 高并发改 WebClient 或专用池

## 相关

[[middleware/gateway-超时与重试配置]] · [[middleware/rpc-超时重试与链路]]
