...
> 概念 [[middleware/io模型与-netty]]；Dubbo 传输层 [[middleware/dubbo-调用原理与分层]]。
...
[[spring/spring-cloud-gateway]] 基于 **Reactor Netty**（WebFlux），与 Netty 原生 API 不同层，但同为 Reactor 非阻塞模型。Filter 里阻塞调用 JDBC 会导致性能骤降。
