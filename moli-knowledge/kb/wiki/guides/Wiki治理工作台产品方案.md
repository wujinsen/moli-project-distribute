...
| 状态 | 触发 |
|------|------|
| `linted` | `POST /kb/wiki/lint-space` |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/kb/wiki/lint-space` | T16a ✅ |
| GET | `/kb/wiki/govern/options` | kind 分类 + 模型列表 |
| POST | `/kb/wiki/govern/script-fix` | T16e ✅ |
| POST | `/kb/wiki/govern/ai-batch-fix` | T16e ✅ |
| POST | `/kb/wiki/govern/auto-fix` | T16e ✅ |
| POST | `/kb/wiki/govern/merge-hint` | T16g ✅ |
| POST | `/kb/sync/trigger` | 可选 Sync |

单页 diff 预览仍可用 `POST /kb/wiki/ai-revise` + `PUT /kb/wiki/page`（T14）。
