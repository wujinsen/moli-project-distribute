...
| 慢接口 | Druid SQL 统计 / [[mysql-深分页与慢sql优化]] |
| 连接泄漏 | `remove-abandoned` 日志；未关 Connection |
| 事务不回滚 | [[spring-声明式事务]] 传播/自调用 |

loadtest 已暴露 **Prometheus Druid 指标**（`druid.pool.*`），见 [[druid连接池与监控]]、[[压测监控与prometheus]]。

## 5. 与分布式组件

- **Redis**：Session/缓存，减轻 DB 读（[[redis-缓存]]）
- **秒杀**：热点在 Redis；订单落库仍靠 Mapper + 合理索引
- **分布式锁 DB 方案**：长事务占连接 → 易打满池（[[分布式锁]]）

## 6. 编码规范（项目级）

- 动态 SQL 值用 `#{}`，禁止随意 `${}`（[[mybatis-plus-用法与注入防护]]）
