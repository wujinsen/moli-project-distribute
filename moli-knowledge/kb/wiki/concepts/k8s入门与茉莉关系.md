...
**Kubernetes** 编排容器：部署、扩缩容、服务发现、滚动升级。茉莉当前以**本地/单机 + Docker Compose** 为主；K8s 用于**压测分布式 k6** 或未来生产上云。
...
| 组件 | K8s 部署思路 |
|------|----------------|
| user-center / order / gateway | 各一个 Deployment + Service |
| Nacos / MySQL / Redis | 有状态需 PVC 或云托管 |
| 压测 k6 | 仓库已有 `load-test/k8s/k6-seckill-testrun.yaml` [[秒杀压测指南]] |

## 3. 与 Docker 区别

- Docker：单机跑容器。
- K8s：多节点调度、自愈、HPA 扩缩容。
- 入门先掌握 [[容器与-docker]]，再上 K8s。
...
[[docker部署指南]] · [[jenkins-ci入门]] · [[服务调用与架构]]
