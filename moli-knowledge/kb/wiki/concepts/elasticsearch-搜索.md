...
| 深分页 | 需优化 [[mysql-深分页与慢sql优化]] | scroll/search_after |
| 聚合统计 | SQL GROUP BY | aggregations |

`LIKE '%keyword%'` 无法走 B+Tree 索引（[[mysql-索引失效场景]]），高频搜索应考虑 ES 或 Meilisearch。
...
1. 概念与写入 refresh/flush → [[es-索引与写入流程]]
2. 搜索两阶段与分片路由 → [[es-搜索与分片路由]]
3. match/bool/boost → [[es-match与bool查询]]
4. 面试速记 → [[elasticsearch-面试题]]
