...
> 算法基础 [[限流算法与令牌桶]]；接入步骤 [[sentinel-接入与规则配置]]；Gateway [[spring-cloud-gateway]]；面试 [[sentinel-面试题]]。
...
统计基于 **滑动窗口**（`StatisticSlot` + `LeapArray`），见 [[sentinel-接入与规则配置]] 原理摘要。
...
| **依赖** | 对 MySQL 慢调用间接保护：Druid 池 + 限流（见 [[druid连接池与监控]]） |
...
分布式限流也可用 Redis+Lua 实现计数（见 [[限流算法与令牌桶]]），Sentinel 提供控制台、规则持久化与熔断一体化。
