...
> 事件解耦 [[spring-事件机制]]；线程池参数 [[线程池-实战调优]]；编排 [[completablefuture-异步编排]]。
...
- 默认代理：**Spring AOP**，同类自调用不生效（同 [[spring-aop与代理]] 自调用陷阱）
...
- **ThreadLocal 丢失**：Shiro Session、traceId → [[threadlocal-与上下文传递]] [[mdc-日志链路上下文]]
