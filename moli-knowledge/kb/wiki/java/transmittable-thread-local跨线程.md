...
> ThreadLocal 基础 [[security/threadlocal-与上下文传递]]；MDC [[java/mdc-日志链路上下文]]；@Async [[spring/spring-async与线程池]]。
...
`RpcContext` attachment 在 consumer/provider 间传递；异步调用需 TTL 或显式 `setAttachment`，否则链路断 [[middleware/rpc-超时重试与链路]]。
