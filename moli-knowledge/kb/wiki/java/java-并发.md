...
> JMM 与 happens-before 见 [[java/jmm与happens-before]]；synchronized 见 [[java/synchronized与锁原理]]；volatile 见 [[java/volatile与可见性]]；ConcurrentHashMap 见 [[java/concurrenthashmap原理]]；面试速记 [[java/java-并发面试题]]。
...
| 跨 JVM 互斥 | [[cache/分布式锁]]（Redis/ZK），不是 synchronized |

## 常见坑

1. **`i++` 非原子**：用 AtomicInteger 或 synchronized/Lock。
2. **先删缓存再写库**：脏读概率高，见 [[cache/cache-aside与缓存更新模式]]。
