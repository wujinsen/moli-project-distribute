...
> 枢纽 [[elasticsearch-搜索]]；写入 [[es-索引与写入流程]]；搜索 [[es-搜索与分片路由]]。
...
协调节点算 shard → 主分片写 Memory Buffer + translog → refresh 可搜 → flush 落盘清 translog。见 [[es-索引与写入流程]]。
...
**Query Then Fetch**：各分片 query 得 id+sort → 协调节点合并 → fetch `_source`。近实时因 refresh。见 [[es-搜索与分片路由]]。
