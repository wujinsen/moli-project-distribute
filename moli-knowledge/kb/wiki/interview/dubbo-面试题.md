...
> [[dubbo-与-nacos]] [[dubbo-调用原理与分层]] [[nacos-注册与配置]]
...
Business / RPC(config,proxy,registry,cluster) / Remoting(protocol,transport,serialize)。见 [[dubbo-调用原理与分层]]。
...
Nacos 作 Registry；Dubbo 2.7+ 原生集成 Nacos。茉莉用 Nacos + Dubbo。

## Q9. 与 Spring Cloud OpenFeign 区别？

Dubbo RPC 二进制、性能高、接口契约；Feign HTTP REST。茉莉已选 Dubbo。

## Q10. No provider 怎么排？

Provider 是否起、group/version、namespace、防火墙、Nacos 是否 8848。见 [[故障排查指南]]。
