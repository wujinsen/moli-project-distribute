...
> 枢纽 [[io模型与-netty]]；BIO/NIO [[bio-nio-aio对比]]；Reactor [[netty-reactor与线程模型]]。

## Q1. BIO 和 NIO 区别？

BIO 阻塞 IO，一连接一线程。NIO 非阻塞 + Channel/Buffer/Selector 多路复用，少线程处理多连接。见 [[bio-nio-aio对比]]。
...
单 Reactor 单线程；单 Reactor 多线程；**主从 Reactor**（Boss accept + Worker IO）。Netty 服务端默认主从。见 [[netty-reactor与线程模型]]。
...
定长、分隔符、**长度字段**帧协议；Pipeline 加 FrameDecoder。见 [[netty-pipeline与编解码]]。
...
Dubbo 默认 **Netty** 作为 RPC 传输层，Provider/Consumer 间 TCP 长连接多路复用。见 [[dubbo-调用原理与分层]]。
...
Path/Files、WatchService、AsynchronousFileChannel 等。见 [[bio-nio-aio对比]]。
