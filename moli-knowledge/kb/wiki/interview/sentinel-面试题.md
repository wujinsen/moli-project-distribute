...
> 枢纽 [[sentinel-限流与熔断]]；算法 [[限流算法与令牌桶]]；接入 [[sentinel-接入与规则配置]]。

## Q1. 限流、降级、熔断区别？

**限流**：控制进入速率/并发，防备流量过大。**降级**：关闭或简化非核心能力，返回兜底。**熔断**：依赖故障时快速失败，停止调用坏下游。见 [[sentinel-限流与熔断]]。

## Q2. 令牌桶和漏桶区别？

令牌桶按固定速率放令牌，允许突发（桶内有令牌即可）。漏桶固定速率流出，平滑输入。Guava RateLimiter 属令牌桶。见 [[限流算法与令牌桶]]。
...
`StatisticSlot` 用 `LeapArray` 分桶统计 pass/block/RT/exception，供 Flow/Degrade 规则判断。见 [[sentinel-接入与规则配置]]。
...
Redis+Lua 原子计数、Sentinel 集群流控、Gateway 统一 QPS。秒杀库存用 Redis Lua（[[redis-面试题]]），接口 QPS 常用 Sentinel/nginx。
