...
> Tomcat 枢纽 [[tomcat与-servlet容器]]；请求链 [[servlet生命周期与请求流程]]。

## Q1. GET 和 POST 区别？

GET 幂等、取资源，参数在 URL；POST 可能改服务器状态，body 传参。GET 长度受 URL/浏览器限制；POST 理论无规范上限。茉莉登录用 **POST** JSON body。
...
init → 多次 service → destroy。见 [[servlet生命周期与请求流程]]。

## Q4. Filter 和 Servlet 区别？

Filter 链式拦截，Servlet 处理请求。Shiro 鉴权在 Filter 层 [[shiro-鉴权体系]]。
...
Spring MVC 前端控制器，分发到 `@Controller`。见 [[spring-mvc请求流程]]。

## Q7. Session 存在哪？茉莉呢？

传统可存 Tomcat 内存；集群需 Redis 等。茉莉 **Session 在 Redis**，非 Tomcat 复制 [[认证与会话机制]]。

## Q8. Cookie 和 Session？

Cookie 客户端；Session 服务端。茉莉 token 走 **Authorization 头**，非 Cookie [[登录与鉴权指南]]。
...
Gateway 用 **Netty/WebFlux**；业务用 **Tomcat Servlet 阻塞模型** [[spring-cloud-gateway]]、[[io模型与-netty]]。

## Q11. 404 可能在哪一层？

Gateway 路由错、StripPrefix 错、Tomcat context-path、Controller 映射错。分层排查 [[故障排查指南]]。
