...
> [[mysql-事务与锁]] [[mysql-隔离级别与mvcc]] [[mysql-innodb锁机制]] [[mysql-死锁与排查]]；Spring 层 [[spring-事务]] [[spring-声明式事务]]。

## Q1. ACID 分别是什么？

原子 undo、一致业务约束、隔离并发、持久 redo。见 [[mysql-事务与锁]]。
...
多版本 + Read View + undo；快照读不加锁。见 [[mysql-隔离级别与mvcc]]。
...
加锁顺序不一致；InnoDB 选一方回滚；应用捕获 1213 重试；固定加锁顺序。见 [[mysql-死锁与排查]]。
