...
> 容器概念 [[tomcat与-servlet容器]]；Spring MVC [[spring-mvc请求流程]]。
...
Tomcat 从 **线程池** 取线程执行整个 Filter + Servlet 链。长耗时 JDBC/Dubbo 会占住线程 → 高并发时需调 `max-threads` 与连接池（[[druid连接池与监控]]）。

与 Netty **EventLoop 非阻塞**对比见 [[io模型与-netty]]。
