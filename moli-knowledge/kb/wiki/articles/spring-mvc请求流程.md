...
浏览器 → **Spring Cloud Gateway**（WebFlux）→ HTTP 转发 → 下游 **DispatcherServlet**（Servlet 栈）。两层「分发器」，不要混淆，见 [[spring-cloud-gateway]]。

## 与 Shiro 过滤器

Shiro `Filter` 链在 DispatcherServlet **之前**拦截，未认证请求到不了 Controller，见 [[认证与会话机制]]。
