...
> 内存/GC 概念 [[jvm-内存与gc]]；OOM 排查 [[jvm-oom与排查入门]]；启停脚本 [[生产环境服务启停脚本]]。

摘自历史生产运维笔记（JDK 8 + CMS 时代），供理解**大堆 + 详细 GC 日志 + OOM dump** 的配置思路；**新部署优先 G1**（见 [[jvm-垃圾收集算法与收集器]]）。
...
- CPU 100% / 线程：`jstack` → [[jvm-oom与排查入门]]
