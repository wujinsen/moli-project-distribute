...
> 结构背景 [[b-plus树与-innodb索引结构]]；枢纽 [[mysql-索引]]。
...
若常查 `WHERE a=?` 且只 `SELECT b,c`，可考虑 `INDEX(a,b,c)` 实现覆盖（仍受最左前缀约束，见 [[mysql-复合索引与最左前缀]]）。

## 索引下推 ICP（补充）

MySQL 5.6+ **Index Condition Pushdown**：联合索引中，即使无法覆盖全部 WHERE，也可在存储引擎层先过滤索引列，再回表，减少回表行数（面试加分项，见 [[mysql-索引面试题]]）。
