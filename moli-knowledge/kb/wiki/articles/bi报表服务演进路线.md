...
| 对齐持久层 | 引入 MyBatis-Plus + Druid，与用户中心一致 [[mybatis-与-druid持久层]] |
| 健康检查 | `/actuator/health` 可选暴露 |

## 阶段 1：只读报表 API

- 只读 MySQL 账号连接 `moli` 库
- 指标 API：分页、时间范围、固定 SQL + 索引 [[mysql-索引]]
- 全走 `@RequiresPermissions`，权限走 RBAC [[rbac-权限模型]]
...
| 需求 | 技术 |
|------|------|
| 模糊搜日志/文档 | [[elasticsearch-搜索]] |
| 大数据导出 | 异步任务 + MinIO（参考 [[minio-附件存储指南]]） |
| 实时大屏 | Redis 缓存热点聚合 [[redis-缓存]] |
...
- 在 BI 内做登录（应禁止，走 Starter）[[shiro-starter与跨服务校验]]
- 报表 SQL 无索引全表扫拖垮主库
- 与订单写库共用高权限 DB 账号
