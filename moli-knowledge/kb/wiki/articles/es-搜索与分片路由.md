...
> 概念 [[elasticsearch-搜索]]；DSL [[es-match与bool查询]]。
...
分词 → term → 倒排表（posting list）→ 文档 id 集合。Lucene 底层 FST/Timeline 等结构见官方「索引文件格式」；面试知道 **term → 文档列表** 即可（[[elasticsearch-面试题]]）。
