...
![image 5](<无标题笔记(同步发生冲突).note_images/imageFile5.png>)

创建⽤户好后，重新登陆创建的⽤户

![image 6](<无标题笔记(同步发生冲突).note_images/imageFile6.png>)
...
![image 7](<无标题笔记(同步发生冲突).note_images/imageFile7.png>)

host仓库这⾥了两种不同的存储类型

![image 8](<无标题笔记(同步发生冲突).note_images/imageFile8.png>)

![image 9](<无标题笔记(同步发生冲突).note_images/imageFile9.png>)

#### （3）group仓库 主要就是把上⾯三个仓库放到该组⾥，然后让这个组给外⾯访问。

![image 10](<无标题笔记(同步发生冲突).note_images/imageFile10.png>)

### 全家福来⼀个

![image 11](<无标题笔记(同步发生冲突).note_images/imageFile11.png>)
...
![image 12](<无标题笔记(同步发生冲突).note_images/imageFile12.png>)
