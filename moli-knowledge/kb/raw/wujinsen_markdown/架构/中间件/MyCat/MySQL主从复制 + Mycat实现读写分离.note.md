...
![image 1](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile1.png>)
...
![image 2](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile2.png>)

复制代码

截图

![image 3](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile3.png>)
...
![image 4](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile4.png>)
...
![image 5](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile5.png>)

复制代码

截图

![image 6](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile6.png>)
...
![image 7](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile7.png>)
...
![image 8](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile8.png>)

执⾏成功！

开启从服务器start slave;

![image 9](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile9.png>)

查看从服务器连接状态show slave status\G /*!不加分号*/

![image 10](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile10.png>)

Sqlyog不⽀持\解析不了sql可以使⽤命令⾏来查看 使⽤账号：yingyue 进⾏远程登录到192.168.42.129 上

![image 11](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile11.png>)

全部信息如下：

![image 12](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile12.png>)
...
![image 13](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile13.png>)
...
![image 14](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile14.png>)

看看从服务器192.168.42.129 上从库test_db是否会⾃动同步country表

![image 15](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile15.png>)

没问题 下⾯来删除主192.68.42.128上主库test_db的country表

![image 16](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile16.png>)

看看192.168.42.129 上的从库的情况

![image 17](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile17.png>)

也是可以的，没问题，现在再重新创建表country 下⾯测试需要使⽤（在主库中创建）

![image 18](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile18.png>)
...
![image 19](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile19.png>)

查看192.168.42.129的test_db从库的country表是否多了11条记录

![image 20](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile20.png>)

没问题。 在192.168.42.128的主库test_db上执⾏update country set name='中华⼈⺠共和国' where id = 5;

![image 21](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile21.png>)

查看192.168.42.129的test_db从库的country表

![image 22](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile22.png>)

没问题。 在192.168.42.128的主库test_db上执⾏ delete from country where id = 11;

![image 23](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile23.png>)

查看192.168.42.129的test_db从库的country表，最后⼀个 ‘⽐利时’应该没了

![image 24](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile24.png>)

没问题。

读写分离（主库写， 从库读） 读写分离使⽤的中间件为mycat 安装mycat 过程略 官⽅参考⽂档：

![image 25](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile25.png>)

配置mycat环境变量 略： 注意防⽕墙开启8066 和9066 端⼝

![image 26](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile26.png>)

修改server.xml⽂件

![image 27](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile27.png>)

编辑server.xml⽂件（读写分离这些就够了，多了碍眼）

![image 28](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile28.png>)
...
![image 29](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile29.png>)

复制代码

配置schema.xml⽂件 vi conf/schema.xml

![image 30](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile30.png>)
...
![image 31](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile31.png>)

复制代码

启动mycat 进⼊mycat安装怒⽬执⾏/bin/mycat start 或 /bin/mycat console

![image 32](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile32.png>)

如下启动完成

![image 33](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile33.png>)

使⽤mycat作为代理连接MySQL数据库

![image 34](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile34.png>)
...
![image 35](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile35.png>)

看到数据是从dn1节点查的。 插⼊⼀条数据

![image 36](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile36.png>)

下⾯来看看mycat监控⽇志

![image 37](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile37.png>)

从⽇志看到最终被插⼊到192.168.42.128的MySQL的test_db的country中 看⼀下主库是否有添加的数据

![image 38](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile38.png>)

果然有，说明读写分离成功! 然后从库192.168.42.129中的test_db会复制这条数据

![image 39](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile39.png>)

没问题的，读写分离也是成功的。

题外话：之前以为mycat可以实现主从库数据复制，原来是不可以的，看看下⾯，其实mycat就是代理实现了主从库的读写分离。总 结：主从复制与读写分离⽆关，主从复制是MySQL的，读写分离是Mycat的。

![image 40](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile40.png>)

-------------------------------------------------------------------------------------------------⾼级篇

MySQL的slave断了怎么办？ 如下图：

![image 41](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile41.png>)
...
![image 42](<MySQL主从复制 + Mycat实现读写分离.note_images/imageFile42.png>)
