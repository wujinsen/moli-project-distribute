...
下载地址：htps:/dev.mysql.com/downloads/mysql/5.7.html#downloads
...
groupad mysql userad -r -g mysql mysql 创建数据⽬录并赋予权限

mkdir -p /data/mysql #创建⽬录 chown mysql:mysql -R /data/mysql #赋予权限
...
SET PASWORD = PASWORD('v6|8@I36G@'); ALTER USER 'rot'@'localhost' PASWORD EXPIRE NEVER; FLUSH PRIVILEGES; mysql8: SETPASWORD='v6|8@I36G@';
...
use mysql #访问mysql库 update user set host = '%' where user = 'rot'; #使rot能再任何host访问
