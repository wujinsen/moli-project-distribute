...
![image 1](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile1.png>)
...
![image 2](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile2.png>)

下⾯是UDF的解析代码

![image 3](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile3.png>)

## 定义对应的触发器

![image 4](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile4.png>)

![image 5](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile5.png>)

![image 6](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile6.png>)
...
![image 7](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile7.png>)
...
![image 8](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile8.png>)

例如下⾯是⼀个云数据库实例分析：

云数据库与本地数据库是主从关系。云数据库作为主数据库主要提供写，本地数据库作为从数据库从 主数据库中读取数据 本地数据库读取到数据之后，解析Bin log，然后将数据写⼊写⼊同步到Redis中，然后客户端从Redis读 数据

![image 9](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile9.png>)
...
![image 10](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile10.png>)
...
![image 11](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile11.png>)
...
![image 12](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile12.png>)

更多关于Cancl可以百度搜索 下⾯是运⾏拓扑图

![image 13](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile13.png>)

MySQL表的同步，采⽤责任链模式，每张表对应⼀个Filter 。例如zvsync中要⽤到的类设计如下：

![image 14](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile14.png>)

下⾯是具体化的zvsync中要⽤到的类 ，每当新增或者删除表时，直接进⾏增删就可以了

![image 15](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile15.png>)
...
![image 16](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile16.png>)

![image 17](<MySQL 与 Redis 缓存的同步方案.note_images/imageFile17.png>)
