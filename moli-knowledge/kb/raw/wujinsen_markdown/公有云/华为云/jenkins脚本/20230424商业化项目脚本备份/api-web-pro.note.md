...
- 6 final def poolId = "30821eaf-d1b5-44dc-823e-b6d7432588e2"; // 后端服务器组id

- 7 final def imageHost = "172.16.18.122"; //123.249.100.63 镜像服务器
...
scp /var/lib/jenkins/workspace/api-web-pro/statsweb/target/*.war root@${serverList[i].address.trim()}:/home/xinwu/www/xwuadapi/wars

- 161

ssh -p 22 -o 'StrictHostKeyChecking=no' root@${serverList[i].address.trim()} '/home/xinwu/www/xwuad-api/startup.sh'
...
- 172 if(AsServer=="true"){

- 173

- 174 println("start 发布弹性服务")

- 175 //修改权重为0

- 176 println("start 修改权重为0")

- 177


- 178 for (int i = 0; i < asServerList.size(); i++) {
...
- 181 println weightUrl

- 182 println weightReqBody

def responseWeight = httpRequest contentType: 'APPLICATION_JSON', httpMode: 'PUT', requestBody: weightReqBody, customHeaders: [ [name: "X-Auth-Token", value: responseToken] ], url: weightUrl

- 183

- 184 if(responseWeight.status){

- 185

- 186 println(asServerList[i].address+"成功修改权重为0")

- 187 echo "拷⻉jar包⾄服务器: ${imageHost} "
...
- 191 echo "开始scp拷⻉jar包⾄服务器: ${asServerList[i].address}"
...
- 195 echo "${asServerList[i].address} 启动项⽬完成"
...
- 199 println(asServerList[i].address+"修改权重为0失败")
...
- 204 println("end 修改权重为0")

- 205

- 206 //修改权重为10

- 207 for (int i = 0; i < asServerList.size(); i++) {
...
- 210 println weightUrl

- 211 println weightReqBody
...
- 213 if(responseWeight.status){

- 214 println(asServerList[i].address+"成功修改权重为10")

- 215 }else{

- 216 println(asServerList[i].address+"修改权重为10失败")
...
- 221 println("end 修改权重为10")

- 222

- 223 println("end 发布弹性服务")
...
- 238 println("start 修改权重为10")

- 239 def weightReqTen = '''
...
- 247 for (int i = 0; i < serverList.size(); i++) {
...
- 250 println weightUrl

- 251 println weightReqBodyTen

def responseWeight = httpRequest contentType: 'APPLICATION_JSON', httpMode: 'PUT', requestBody: weightReqBodyTen, customHeaders: [ [name: "X-Auth-Token", value: responseToken] ], url: weightUrl

- 252

- 253 if(responseWeight.status){

- 254 println(serverList[i].address+"成功修改权重为10")

- 255 }else{

- 256 println(serverList[i].address+"修改权重为10失败")

- 257 }

- 258 }

- 259 println("end 修改权重为10")
...
- 269 wrap([$class: 'BuildUser']) {

- 270 buildName "#${BUILD_NUMBER} -- ${BUILD_USER}"
...
- 13 /home/xinwu/www/xwuad-api/bin/shutdown.sh
...
- 22 /home/xinwu/www/xwuad-api/bin/startup.sh
