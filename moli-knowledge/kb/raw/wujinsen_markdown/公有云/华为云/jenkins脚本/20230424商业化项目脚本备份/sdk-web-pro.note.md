...
- 6 final def poolId = "746397c6-74e4-4a1b-b644-c2f718203b06"; // 后端服务器组id

- 7 final def imageHost = "172.16.1.186"; //123.249.100.63 镜像服务器
...
scp /var/lib/jenkins/workspace/sdk-web-pro/sdkweb/target/*.war root@${serverList[i].address.trim()}:/home/xinwu/www/xwuadsdk/wars

- 161

ssh -p 22 -o 'StrictHostKeyChecking=no' root@${serverList[i].address.trim()} '/home/xinwu/www/xwuad-sdk/startup.sh'
...
scp -P 52077 -o 'StrictHostKeyChecking=no' /home/xinwu/www/xwuad-sdk/wars/sdk-web.war root@${asServerList[i].address}:/home/xinwu/www/xwuad-sdk/wars/

- 191

ssh -p 52077 -o 'StrictHostKeyChecking=no' root@${asServerList[i].address} 'kill -9 8080; daemonize /home/xinwu/www/xwuadsdk/bin/startup.sh'

- 192

- 193 echo "${asServerList[i].address} 启动项⽬完成"

""", execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/home/xinwu/www/xwuad-sdk/wars', remoteDirectorySDF: false, removePrefix: 'sdkweb/target/', sourceFiles: 'sdk-web/target/*.war', usePty: true)], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: true)])
...
- 197 println(asServerList[i].address+"修改权重为0失败")
...
- 202 println("end 修改权重为0")

- 203

- 204 //修改权重为10

- 205 for (int i = 0; i < asServerList.size(); i++) {
...
- 208 println weightUrl

- 209 println weightReqBody
...
- 211 if(responseWeight.status){

- 212 println(asServerList[i].address+"成功修改权重为10")

- 213 }else{

- 214 println(asServerList[i].address+"修改权重为10失败")
...
- 219 println("end 修改权重为10")

- 220

- 221 println("end 发布弹性服务")
...
- 236 println("start 修改权重为10")

- 237 def weightReqTen = '''
...
- 245 for (int i = 0; i < serverList.size(); i++) {
...
- 248 println weightUrl

- 249 println weightReqBodyTen

def responseWeight = httpRequest contentType: 'APPLICATION_JSON', httpMode: 'PUT', requestBody: weightReqBodyTen, customHeaders: [ [name: "X-Auth-Token", value: responseToken] ], url: weightUrl

- 250

- 251 if(responseWeight.status){

- 252 println(serverList[i].address+"成功修改权重为10")

- 253 }else{

- 254 println(serverList[i].address+"修改权重为10失败")

- 255 }

- 256 }

- 257 println("end 修改权重为10")
...
- 267 wrap([$class: 'BuildUser']) {

- 268 buildName "#${BUILD_NUMBER} -- ${BUILD_USER}"
...
- 13 /home/xinwu/www/xwuad-sdk/bin/shutdown.sh
...
- 22 /home/xinwu/www/xwuad-sdk/bin/startup.sh
