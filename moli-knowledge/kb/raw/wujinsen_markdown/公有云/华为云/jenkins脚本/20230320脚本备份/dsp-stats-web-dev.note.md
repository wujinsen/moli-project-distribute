...
- 6 final def poolId = "db8c9c2f-9ab3-45bd-894a-310362c8f56b"; // 后端服务器组id

- 7 final def imageHost = "172.16.14.22"; // 123.249.15.47 镜像服务器
...
- 169 /home/xinwu/www/dsp-stats/startup.sh restart

echo "启动项⽬完成" ''', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/home/xinwu/www/dsp-stats/', remoteDirectorySDF: false, removePrefix: 'dsp-stats-web/target/', sourceFiles: 'dsp-statsweb/target/*.jar', usePty: true)], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: true)])
...
- 179 println("start 发布弹性服务")

- 180 //修改权重为0

- 181 println("start 修改权重为0")

- 182 for (int i = 0; i < asServerList.size(); i++) {
...
- 185 println weightUrl

- 186 println weightReqBody

def responseWeight = httpRequest contentType: 'APPLICATION_JSON', httpMode: 'PUT', requestBody: weightReqBody, customHeaders: [ [name: "X-Auth-Token", value: responseToken] ], url: weightUrl

- 187

- 188 if(responseWeight.status){

- 189

- 190 println(asServerList[i].address+"成功修改权重为0")

- 191 echo "拷⻉jar包⾄服务器: ${imageHost} "
...
- 195 echo "开始scp拷⻉jar包⾄服务器: ${asServerList[i].address}"
...
- 198 echo "${asServerList[i].address} 启动项⽬完成"

""", execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/home/xinwu/www/dsp-stats/', remoteDirectorySDF: false, removePrefix: 'dspstats-web/target/', sourceFiles: 'dsp-stats-web/target/*.jar', usePty: true)], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: true)])
...
- 202 println(asServerList[i].address+"修改权重为0失败")
...
- 207 println("end 修改权重为0")

- 208

- 209 //修改权重为10

- 210 for (int i = 0; i < asServerList.size(); i++) {
...
- 213 println weightUrl

- 214 println weightReqBody
...
- 216 if(responseWeight.status){

- 217 println(asServerList[i].address+"成功修改权重为10")

- 218 }else{

- 219 println(asServerList[i].address+"修改权重为10失败")
...
- 224 println("end 修改权重为10")

- 225

- 226 println("end 发布弹性服务")
...
- 241 println("start 修改权重为10")

- 242 def weightReqTen = '''
...
- 250 for (int i = 0; i < serverList.size(); i++) {
...
- 253 println weightUrl

- 254 println weightReqBodyTen

def responseWeight = httpRequest contentType: 'APPLICATION_JSON', httpMode: 'PUT', requestBody: weightReqBodyTen, customHeaders: [ [name: "X-Auth-Token", value: responseToken] ], url: weightUrl

- 255

- 256 if(responseWeight.status){

- 257 println(serverList[i].address+"成功修改权重为10")

- 258 }else{

- 259 println(serverList[i].address+"修改权重为10失败")

- 260 }

- 261 }

- 262 println("end 修改权重为10")
...
- 272 wrap([$class: 'BuildUser']) {

- 273 buildName "#${BUILD_NUMBER} -- ${BUILD_USER}"
