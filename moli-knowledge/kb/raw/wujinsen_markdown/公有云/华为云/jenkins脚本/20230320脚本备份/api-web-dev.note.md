...
- 6 final def poolId = "8ecaa955-eb70-4723-a888-70e55c86e906"; // 后端服务器组id

- 7 final def imageHost = "172.16.2.254"; //123.249.100.63 镜像服务器
...
- 161 kill -9 `lsof -i:8080 | awk 'NR==2{print $2}'`

- 162 daemonize /home/xinwu/www/xwuad-api/bin/startup.sh

echo "启动项⽬完成" ''', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/home/xinwu/www/xwuad-api/wars', remoteDirectorySDF: false, removePrefix: 'api-web/target/', sourceFiles: 'api-web/target/*.war', usePty: true)], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: true)])

- 163

- 164 println("end 发布固定服务器")
...
- 171 if(AsServer=="true"){

- 172

- 173 println("start 发布弹性服务")

- 174 //修改权重为0

- 175 println("start 修改权重为0")


- 176

- 177 for (int i = 0; i < asServerList.size(); i++) {
...
- 180 println weightUrl

- 181 println weightReqBody

def responseWeight = httpRequest contentType: 'APPLICATION_JSON', httpMode: 'PUT', requestBody: weightReqBody, customHeaders: [ [name: "X-Auth-Token", value: responseToken] ], url: weightUrl

- 182

- 183 if(responseWeight.status){

- 184

- 185 println(asServerList[i].address+"成功修改权重为0")

- 186 echo "拷⻉jar包⾄服务器: ${imageHost} "
...
- 190 echo "开始scp拷⻉jar包⾄服务器: ${asServerList[i].address}"

scp -P 52077 -o 'StrictHostKeyChecking=no' /home/xinwu/www/xwuad-api/wars/api-web.war root@${asServerList[i].address}:/home/xinwu/www/xwuad-api/wars/

- 191

ssh -p 52077 -o 'StrictHostKeyChecking=no' root@${asServerList[i].address} 'kill -9 8080;daemonize /home/xinwu/www/xwuadapi/bin/startup.sh'

- 192

- 193

- 194 echo "${asServerList[i].address} 启动项⽬完成"

""", execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/home/xinwu/www/xwuad-api/wars', remoteDirectorySDF: false, removePrefix: 'apiweb/target/', sourceFiles: 'api-web/target/*.war', usePty: true)], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: true)])
...
- 198 println(asServerList[i].address+"修改权重为0失败")
...
- 203 println("end 修改权重为0")

- 204

- 205 //修改权重为10

- 206 for (int i = 0; i < asServerList.size(); i++) {
...
- 209 println weightUrl

- 210 println weightReqBody
...
- 212 if(responseWeight.status){

- 213 println(asServerList[i].address+"成功修改权重为10")

- 214 }else{

- 215 println(asServerList[i].address+"修改权重为10失败")
...
- 220 println("end 修改权重为10")

- 221

- 222 println("end 发布弹性服务")
...
- 237 println("start 修改权重为10")

- 238 def weightReqTen = '''
...
- 246 for (int i = 0; i < serverList.size(); i++) {
...
- 249 println weightUrl

- 250 println weightReqBodyTen

def responseWeight = httpRequest contentType: 'APPLICATION_JSON', httpMode: 'PUT', requestBody: weightReqBodyTen, customHeaders: [ [name: "X-Auth-Token", value: responseToken] ], url: weightUrl

- 251

- 252 if(responseWeight.status){

- 253 println(serverList[i].address+"成功修改权重为10")

- 254 }else{

- 255 println(serverList[i].address+"修改权重为10失败")

- 256 }

- 257 }

- 258 println("end 修改权重为10")
...
- 268 wrap([$class: 'BuildUser']) {

- 269 buildName "#${BUILD_NUMBER} -- ${BUILD_USER}"
