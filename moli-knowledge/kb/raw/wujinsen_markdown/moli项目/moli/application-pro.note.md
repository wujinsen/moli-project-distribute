...
8&autoReconect=true&serverTimezone=Asia/Shanghai driver-clas-name:com.mysql.cj.jdbc.Driver username: rot pasword: mWDQ34IPDq

# druid 配 置 信 息 druid:

min-idle: 3 max-active: 15 initial-size: 5 min-evictable-idle-time-milis: 3 0 max-evictable-idle-time-milis: 6 0 validation-query: SELECT 1
...
pol: max-active:1 0# 连 接 池 最 ⼤ 连 接数 （ 使 ⽤ 负 值 表 示 没 有 限 制 ） max-wait: -1 # 连 接 池 最 ⼤ 阻 塞 等 待 时 间 （ 使 ⽤ 负 值 表 示 没 有 限 制 ） max-idle:10 # 连 接 池 中 的 最 ⼤ 空 闲 连 接 min-idle:5 # 连 接 池 中 的 最 ⼩ 空 闲 连 接

database:1

mybatis-plus: # 配 置 映 射 ⽂ 件位 置 ， claspath指 resources maper-locations: claspath*:maper/*.xml# maper.xml⽂ 件 所 在 位 置

# 打 印 sql到 控 制 台 configuration:
...
minio: url: htp:/localhost:9 0 acesKey: minioadmin secretKey: minioadmin
