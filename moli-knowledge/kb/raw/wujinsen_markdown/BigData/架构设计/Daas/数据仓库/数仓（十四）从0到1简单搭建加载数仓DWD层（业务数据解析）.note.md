...
![image 1](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile1.png>)
...
![image 2](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile2.png>)

- 6个维度表的退化操作其实我们在前⾯的第⼗⼆章节已经做了即DIM层。除了第3张表即商品维度表是5 个表退化到1张表上，其他都是1-2张表退化到1张表上，相对⽐较简单。
- 2.4、确认事实 就是确认事实表的每张事实表的度量值。


![image 3](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile3.png>)
...
![image 4](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile4.png>)
...
![image 5](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile5.png>)
...
![image 6](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile6.png>)
...
![image 7](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile7.png>)
...
![image 8](<数仓（十四）从0到1简单搭建加载数仓DWD层（业务数据解析）.note_images/imageFile8.png>)
