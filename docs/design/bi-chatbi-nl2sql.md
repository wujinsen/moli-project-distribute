...
> **状态**：design · 2026-07-17（未开工，ai-server 0→1）
...
> **模块 v1 骨架**：[`ai-module-overview.md`](ai-module-overview.md) · [`../api/ai-api.md`](../api/ai-api.md)
> **架构图**：[`docs/diagrams/moli-ai-chatbi-flow.drawio`](../diagrams/moli-ai-chatbi-flow.drawio)
...
`moli-ai/moli-ai-server`（artifactId `moli-ai-server`，Nacos 服务名 `ai-server`，HTTP **1128** / Dubbo **20883**，网关 `/AiServer/**`）目前仅
`AiApplication` + 占位 `AiController`（`GET /demo/test`）。平台已有 order/user 真实业务库，但只能靠人写 SQL 或等报表看数。

### 1.2 目标

把 ai-server 从占位骨架升级为 **ChatBI / NL2SQL Agent**：
...
4. **审计**：每次问答的 NL / SQL / 结果摘要 / 耗时落 `ai_chat_trace`。
5. **鉴权**：Shiro session 由 user-center 签发（与 order 一致），bi 只校验；权限码 `ai:chat:query` / `ai:chat:trace`。
...
![ChatBI NL2SQL 调用链](../diagrams/png/moli-ai-chatbi-flow.png)

> 源文件：[moli-ai-chatbi-flow.drawio](../diagrams/moli-ai-chatbi-flow.drawio)

### 2.1 分层（Java 壳 + Python Agent sidecar）

| 组件 | 职责 | 技术 |
|------|------|------|
| `ai-server`（Java） | API 入口、Shiro 鉴权、**SQL 白名单/AST 校验**、只读执行、结果缓存、`ai_chat_trace` 审计 | Spring Boot（现有骨架扩展） |
| `ai-agent`（Python sidecar，新增） | schema 检索、NL→SQL 生成、SQL 自纠错重试、结果解读文案、图表类型建议 | FastAPI + LangGraph |
...
```
meiling-ui / Gradio
  → moli-gateway :21000  /AiServer/**（StripPrefix=1）
  → ai-server :1128（Shiro 校验 + ai:chat:query）
      → ai-agent（Python）：schema 检索 → 生成 SQL
      ← 候选 SQL
  → Java：AST 白名单校验 → 只读执行（LIMIT/超时）→ 结果集
  → ai-agent：结果解读 + 图表建议
  → 返回 {sql, rows, chart, explanation, traceId} + 写 ai_chat_trace
```
...
### 5.1 新增审计表 `ai_chat_trace`（DDL 走 `docs/sql/` 迁移基线）
...
| GET | `/bi/chat/trace/{traceId}` | 查看决策链路（`ai:chat:trace`） |
| GET | `/bi/chat/schema` | 可查询的表/字段白名单（脱敏） |

详细契约在实现时补进 [`docs/api/ai-api.md`](../api/ai-api.md)。
...
- [ ] 端到端 Demo：`/AiServer/**` 自然语言 → SQL → 结果 + 图表 + 解读
- [ ] NL2SQL 测试集执行正确率 ≥80%
- [ ] 危险 SQL（写操作/越权/多语句/超量）**100% 拦截**，`ai_chat_trace` 可查
- [ ] 鉴权走 Shiro（session user-center 签发）；`ai:chat:*` 权限码 enforce
...
| v3 | 大屏、订阅推送（见 [`ai-module-overview.md`](ai-module-overview.md) §6） |

---

## 10. 相关

- 模块 v1 骨架：[`ai-module-overview.md`](ai-module-overview.md)
