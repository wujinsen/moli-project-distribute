...
| 字段 | 必填 | 类型 | 契约语义 |
|------|------|------|----------|
| `id` | 是 | string | 唯一；前缀=空间：`M*`=moli-ops-manual、`E*`=enterprise-kb |
...
5. `id` 前缀与 `space` 一致（M/E 对齐 space_code）。
...
- [ ] `golden.jsonl` ≥50 题（理想 100），§2.1 各难度达「最低」列，§2.2 两空间均有样本。
...
| 文件 | 状态 | 说明 |
|------|------|------|
| `moli-knowledge/kb/eval/golden.jsonl` | ✅ | **59** 题（+4 负样本 E14/M39/M40 等）；dirty 错别字 |
...
**§9.6 修订执行摘要**：
- §9.4：新增 N-cross（E14）、N-lexical（M39/M40）各 2 道。
- §9.3/9.5：M21/E08 错别字、E09 中英混+annex 注、E09 note 已标注。
...
### 9.5 修订清单（Composer 下一轮）

1. 补 §9.3 的 `N-cross`（跨空间越界）与 `N-lexical`（词面陷阱）负样本各 1~2 道。
2. 将 2~3 道 dirty 改为含真错别字 / 中英混。
3. E09 `note` 标注 annex 降权预期。
4. **重跑**检索式 + 生成式两份 `baseline-ngram-*.json`，更新 §8 数字与文件名。
5. 完成后回本节，Opus 复核转 `status: done`。

### 9.6 Opus 复核结论（2026-07-17 · PASS → done）

对 §9.5 五项逐条核验（基于仓库实际文件，非仅凭汇报）：

| # | 项 | 核验方式 | 结论 |
|---|----|----------|------|
| 1 | N-cross / N-lexical | 读 golden：E14（enterprise 问 moli 启动）= N-cross；M39（Milvus 集群）、M40（Chroma 分片 SOP）= N-lexical，均 `expect_slugs=[]`+`expect_answerable=false` | ✅ 各 2 道 |
| 2 | dirty 错别字 | M21「微服雾」、E08「sprngboot/配致」、E09 中英混「case」 | ✅ |
| 3 | E09 annex 注 | note 含「annex 降权预期，hit 可能偏低」 | ✅ |
| 4 | 重跑基线 | `reports/` 被 `.gitignore:50` 忽略、本地 JSON 未留存；但**权威记录已入已提交的 `kb/eval/README.md` 基线表**：59 题 检索式/生成式 hit@3=79.17% hit@8=91.67% MRR=0.742 refusal_acc=0%（kw_pass=85%） | ✅（数字以 README 为准） |
| 5 | 复核 | 本节 | ✅ |

**分层复核（59 题）**：easy 15 / paraphrase 15 / dirty 10 / multi-hop 8 / **negative 11**（7→11），两空间占比达标；有效 answerable = 48。

**指标合理性**：hit@8 91.67%（44/48）；`refusal_accuracy=0%` 是 ngram 严格口径的**预期表现**（含 E14 弱 citation FAIL），正是留给 AI-2 hybrid 证明拒答增益的对照点，非缺陷。

**遗留（非阻断，转 AI-2/backlog）**：
- `reports/*.json` 为 gitignore 临时产物，不入库；AI-2 三档对比将于同一会话重跑 ngram 自带对照，不依赖本轮 JSON 留存。README 基线表为长期权威记录。

**验收结论：AI-1 通过，`status: done`。** 交接 AI-2：可直接以扩容 golden 为基准。
