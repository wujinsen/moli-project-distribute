...
各 `moli-user-center`、`moli-order` 等**不需要**单独 `AGENTS.md`；写 `moli-xxx/README.md` + ingest 到 `wiki/services/` 即可。
...
|------|---------------------|--------------------|-------------------|----------|
| **PRD / 产品** | [product/](product/) | `raw/prd/` | `wiki/guides/`（`tags` 含 `product`） | `enterprise-kb` |
| **技术方案 / 架构** | [design/](design/) + [zh-CN/ARCHITECTURE.md](zh-CN/ARCHITECTURE.md) | `raw/design/` | `wiki/concepts/`、`wiki/articles/` | `enterprise-kb` |
| **API 接口** | **[api/](api/)**（权威） | `raw/api/`（可选副本） | `wiki/services/` + ops 指南引用 | — |
| **测试** | [test/](test/) | `raw/test/` | `wiki/guides/`（压测等） | `enterprise-kb` |
| **运维** | [ops/](ops/)（索引） | `raw/ops/` | **`wiki-ops/guides/`**（权威） | `moli-ops-manual` |

**日常修改**：PRD/技术/测试 → 改对应 **`wiki/`** 页；API → 改 **`docs/api/`**；运维 → 改 **`wiki-ops/`**。改完 wiki → `lint.py` → `sync_to_db.py`。
...
API 文档在 **`docs/api/`**，供开发联调，一般不整篇 ingest 进浏览（服务页 `wiki/services/` 可链到契约）。
...
```
   ...
   kb/wiki/guides            moli-order/README           wiki-ops/
   docs/design               → ingest → wiki/services/   docs/api/
```

| 文档 | 放哪 | 谁维护 | 原因 |
|------|------|--------|------|
| **PRD / 产品路线图** | 平台 `kb/wiki/guides/`、`kb/ROADMAP.md` | 产品 + 平台 | 功能跨多个服务 |
| **跨服务架构**（网关、鉴权、部署拓扑） | `docs/zh-CN/` + `kb/wiki/concepts/` | 平台/架构 | 全局一份真相 |
| **单服务设计**（秒杀 Lua、Shiro 接入） | **`moli-xxx/README.md`** + **`wiki/services/xxx`** | **该服务负责人** | 离实现最近；ingest 后可在 Web 浏览 |
| **HTTP 联调契约** | **`docs/api/`** + 各服务 Swagger | 接口变更者 | 前端/测试只认一处清单 |
| **运维 SOP** | **`kb/wiki-ops/`** | 运维/SRE | 启动顺序、DB、Nacos 天然跨服务 |
...
├─ 只跟一个服务的实现强相关？         → 该模块 README，再 ingest → wiki/services/
├─ 给前端/测试联调的 HTTP 契约？       → docs/api/（统一）+ Swagger（运行时）
└─ 怎么部署、排障、开权限？           → wiki-ops/（统一）
```

### 单服务 README 与 wiki 的关系

1. **第一稿**：在 `moli-order/README.md` 等模块内写（随代码 PR 更新）。
2. **汇总浏览**：Agent Ingest 或人工维护 → `kb/wiki/services/订单服务.md`（链到模块 README 作 `sources`）。
3. **不要**：每个服务各写一份《本地启动全家桶》；统一看 `wiki-ops/guides/本地启动指南.md`。
...
| 服务专属测试说明 | `wiki-ops/`、`docs/zh-CN/` |

各仓 README 通过 **Ingest** 汇总到 `wiki/services/`，避免用户在五个 repo 里找文档。

### 与 §「五类文档」的对应

- **偏统一**：产品、跨域技术方案、运维、全局 API 契约  
- **偏各服务**：单服务 README、模块级设计；经 ingest **汇总**到统一知识库供检索  
- **权威规则**不变：浏览看 `wiki*`；API 改 `docs/api/`；运维改 `wiki-ops/`
